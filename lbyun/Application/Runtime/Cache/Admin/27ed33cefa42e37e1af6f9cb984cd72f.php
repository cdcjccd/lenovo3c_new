...
<div class="container-fluid">
	<ul class="nav nav-tabs">
		<li class="active"><a href="">销售产品占比分析</a></li>
	</ul>
	...
	<div class="row-fluid">
		<div class="span12">
			...
			<div id="test" style="width: 900px; height: 500px; float: left; margin-top: 10px;">
			</div>
			<div class="row-fluid">
...
//			alert( $('#searchForm').serialize() )
            $.ajax({
                type: 'post',
                url: "<?php echo U('Admin/SaleAnalyze/product');?>",
...
//                    alert(res)
    ...
    $(document).ready(function(){
        ...
        var day = 7 + '天';
        var url1 = "<?php echo U('Admin/SaleAnalyze/product');?>?day=7";
        pie_ajax(url1,day);
        var url2 = "<?php echo U('Admin/SaleAnalyze/productLine');?>?day=7";
        line_ajax(url2)
        //柱状图右边的通路下拉框
//        $.ajax({
...
//        $('#nameName').children(':first').prop('selected',true)
        ...
        if ($(this).attr('dirc') == 'product') {
            var url = "<?php echo U('Admin/SaleAnalyze/product');?>?day=7"
            pie_ajax(url,day);
        }
        if ($(this).attr('dirc') == 'line') {
//            var routeName = '';
//            var columnarUrl = '';
            var columnarUrl = "<?php echo U('Admin/SaleAnalyze/productLine');?>?day=7"
//            if ($('#nameName').val() != '') {
...
//			}
    ...
    $('.btn30').click(function(){
        ...
        var day = 30 + '天';
        if ($(this).attr('dirc') == 'product') {
            var url = "<?php echo U('Admin/SaleAnalyze/product');?>?day=30"
            pie_ajax(url,day);
        }
        if ($(this).attr('dirc') == 'line') {
            var url = "<?php echo U('Admin/SaleAnalyze/productLine');?>?day=30"
            line_ajax(url);
        }
    })
...
//            ]
    ...
    /*array(2) {
        ["day"] => array(7) {
            [0] => string(11) "2017-07-26 "
                [1] => string(11) "2017-07-25 "
                [2] => string(11) "2017-07-24 "
                [3] => string(11) "2017-07-23 "
                [4] => string(11) "2017-07-22 "
                [5] => string(11) "2017-07-21 "
                [6] => string(11) "2017-07-20 "
        }
        ["info"] => array(11) {
            [1] => array(5) {
                ["name"] => string(9) "意外保"
    ...
    function columnarHtml(data , name)
	{
		...
		{
            singleName += "<td>"+name[i]+"</td>";
		}
		var strName = "<tr><td>时间</td>"+singleName+"<td>总计</td></tr>";
		...
		for (var s = 0 ; s < data.day.length ; s++)
		{
			...
			{
			    singleData += "<td>"+data.info[n].data[s]+"</td>";
