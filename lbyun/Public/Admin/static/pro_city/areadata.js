...
strVarCity += "                          <div class=\"data-result\"><em>最多选择 <strong>8<\/strong> 项<\/em><\/div>";
strVarCity += "                          <div class=\"data-error\" style=\"display: none;\">最多只能选择 8 项<\/div>";
    ...
    //显示下一级
        ...
        if (typeof (relations[$(con).data("code")]) != "undefined") {
            ...
            if (datatype == "duoxuan") {
                ...
                if ($('.data-result span').length > 10000) {
...
function selectitem_area(con) {
    if (datatype == "duoxuan") {
        //多选
        if ($('.data-result span').length >1000) {
            $('.data-error').slideDown();
            setTimeout("$('.data-error').hide()", 1000);
            return false;
        } else {
